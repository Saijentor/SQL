...
select sum(monthly_salary) avg_salary_js from salary s 
join employee_salary es on s.id=es.salary_id 
join roles_employee re on re.employee_id=es.id 
join roles r on r.id=re.role_id 
